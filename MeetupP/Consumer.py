...
import kafka
from MeetupP.RsvpStream import Rsvpstream
from MeetupP.utils import get_dict_val, load_json, datetime_from_epoch
...
group_id = "meetup_rsvp_stream"  # group id of topics

# Kafka Consumer
consumer = KafkaConsumer("meetup", group_id=group_id,
    ...
    if consumer:
        ...
        for message in consumer:
            ...
            try:
               ...
               continue
            result = u' '.join((str(group_topic_names), str(group_city), str(group_country), str(group_id),
                                str(group_name), str(group_lon), str(group_state), str(group_lat))).encode(
                'utf-8').strip()
            print(result)
